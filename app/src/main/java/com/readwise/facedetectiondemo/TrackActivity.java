        ...
        faceDetectManager.setOnTrackListener(new FaceFilter.OnTrackListener() {
            ...
            public void onTrack(final FaceFilter.TrackedModel trackedModel) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Log.d("scottie","onTrack");
                        if(flag){
                            return;
                        }
                        trackedModel.getImageFrame().retain();
                        final Bitmap cropFace = trackedModel.cropFace();
                        ByteArrayOutputStream baos = new ByteArrayOutputStream();
                        cropFace.compress(Bitmap.CompressFormat.JPEG, 100, baos);
